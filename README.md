...
```bash
operator knowledge query "검색어"
operator knowledge query "검색어" --format json
operator knowledge doctor
```

검색은 제목, 태그, 본문, ID를 가중치 기반으로 정렬합니다. `--format json`은 다른 AI 도구가 안정적으로 파싱할 수 있는 구조화 결과를 출력합니다.

### Graphify

```bash
operator graph run --no-label --no-viz
operator graph label
operator graph viz
operator graph open --html
```

`graph run`은 추출/갱신을 담당하고, LLM community labeling과 HTML 시각화는 `label`, `viz` 명령으로 따로 재시도할 수 있습니다.
...
기본 빌드는 빠른 시작을 위한 `onedir` 모드입니다. 휴대용 단일 파일이 필요할 때만 다음 옵션을 사용합니다.

```bash
./scripts/build.sh --mode onefile
```
...
기본값은 개발 가상환경의 `.venv/bin/operator`를 사용하며, 릴리즈 바이너리를 검증하려면 다음처럼 지정합니다.

```bash
OPERATOR_CMD=release/operator_mac/operator ./test_operator.sh
```

테스트는 임시 `OPERATOR_HOME`을 만들고 public `protocols/`를 복사해서 실행하므로 실제 사용자 runtime 상태를 변경하지 않습니다.
...
Ollama 연결 점검을 제외하고 파일/프로토콜 상태만 확인하려면 다음 옵션을 사용합니다.

```bash
operator doctor --skip-ollama
```
...
`docs/AGENT_GUIDE.md`는 개발 저장소의 public `protocols/`만 기준으로 생성됩니다. Runtime home(`OPERATOR_HOME`)에 있는 개인 회선은 공개 가이드에 포함하지 않습니다.
