...
1. `summarize`를 실제 context history 기반으로 수정한다.
2. `test_operator.sh`를 임시 `OPERATOR_HOME` 기반 smoke test로 교체한다.
3. README와 AGENT_GUIDE 생성 로직을 실제 release 경로/회선 목록과 동기화한다.
4. `ContextManager`에 default_factory, atomic write, parse failure warning을 추가한다.

### Phase 2: Make Release Fast And Predictable

1. macOS 기본 빌드를 `onedir`로 전환한다.
2. `onefile`은 별도 portable 타깃으로 유지한다.
3. `--version` fast path를 추가한다.
4. `operator doctor` 명령을 추가해 release path, protocols, knowledge, context, Ollama 연결을 점검한다.

### Phase 3: Harden Agent And Knowledge Workflows

1. ShellExecutor를 approval policy 기반으로 재설계한다.
2. OAKS에 `--format json`, `doctor`, weighted search를 추가한다.
3. protocol compression cache에 source hash와 model metadata를 저장한다.
4. Graphify 단계를 run/label/viz로 분리한다.
