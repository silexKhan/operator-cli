...
echo "[4/6] Building binary with PyInstaller (onefile mode)..."
rm -rf build/
...
if [ "${CI:-}" = "true" ]; then
    ...
    echo "[6/6] Deploying runtime binary to: $RUNTIME_HOME/operator"
    mkdir -p "$RUNTIME_HOME"
    cp release/operator_mac/operator "$RUNTIME_HOME/operator"
